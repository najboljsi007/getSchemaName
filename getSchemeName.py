...
configFilePath = r'getSchemeNameConfig.confile'
configParser.read(configFilePath)


#Get settings values and save it.
try:
    saveHistory = configParser.get('settings', 'saveHistory')
    deleteExtractedFolder = configParser.get('settings', 'deleteExtractedFolder')
    deleteApp = configParser.get('settings', 'deleteApp')
    openHistoryInFinder = configParser.get('settings', 'openHistoryInFinder')
except ConfigParser.NoSectionError:
    print "Config file is corrupted or missing ..."
    sys.exit()
...
#Clear screen and get computer username and change dir
    ...
    workingDirPath = "/Users/" + computer_username + mobileApplicationspath
    ...
    try:
        if appNum == "x" or appNum == "X":
            if openHistoryInFinder != "True":
                os.system("open /Users/" + computer_username + mobileApplicationspath + "URLSchemesHistory.txt")
            else:
...
#Start unziping process
    ...
    extractedFolderPath = "/Users/" + computer_username + mobileApplicationspath + appName + " Extracted"
